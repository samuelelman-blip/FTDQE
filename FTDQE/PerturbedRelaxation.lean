...
theorem perturbed_relaxation_of_weighted_duhamel
    ...
    (hduhamel : ∀ t : ℝ, 0 ≤ t →
      Real.exp (λ * t) * h t ≤
        κ * xnorm + κ * η * ∫ s in (0 : ℝ)..t, Real.exp (λ * s) * h s) :
    ∀ t : ℝ, 0 ≤ t →
      h t ≤ κ * Real.exp (-(λ - κ * η) * t) * xnorm := by
  intro t ht
  let u : ℝ → ℝ := fun s => Real.exp (λ * s) * h s
  ...
  have hexp : 0 < Real.exp (λ * t) := Real.exp_pos _
  have hdiv : h t ≤
      ((κ * xnorm) * Real.exp ((κ * η) * t)) / Real.exp (λ * t) := by
    apply (le_div_iff₀ hexp).2
    simpa [u] using hu_bound
  calc
    h t ≤ ((κ * xnorm) * Real.exp ((κ * η) * t)) / Real.exp (λ * t) := hdiv
    _ = κ * Real.exp (-(λ - κ * η) * t) * xnorm := by
